...
shared.** The all-circular census population sits below onset (0.70 / 0.43 correct
certifications per realisation, lit / vlbi, under its own fresh floors), exactly as
IGNITE-2 and SURFACE found. Replace ONE of the three loud members with an eccentric source
and the count clears onset at e = 0.3 (1.6 / 1.1) and reaches 6–8 per realisation at
e = 0.7 — an order of magnitude over the circular baseline, at fixed census loudness, while
the mix's own floor nearly doubles and its trials term grows ~8×. The surface peaks at
n_ecc = 2 (8.7 / 7.4) and **inverts by n_ecc = 3 at high e** (109 active harmonic slots,
12 % of the 0.4·N·T·ΔF capacity): the capacity-vs-clock trade crosses at ~5–8 % band
occupancy. But the campaign's namesake question resolves NEGATIVE: in 20 exact seed-paired
...
model, provenance in `ch_floors.npz`), Gumbel-MLE α = 0.05, fit error 2.80·β̂/√N, nullN
zero-fraction quoted. **Two floor findings before any signal is scored:**
1. **The comb raises its own bar.** The operative fN grows with the mix (lit tier: m0 4.31 ±
   0.37 → m1e07 8.51 ± 0.72 → m2e07 9.91 ± 0.84 nat; vlbi: 7.59 → 10.64 → 11.88) — the
   confusion cost of harmonic occupancy, measured in the null family itself. (Exception: the
   n_ecc=3 high-e cells' floors *fall* again (m3e07 7.65/7.72) — at 109 active slots the
...
2. **fALL inverts under the comb.** In every eccentric lit cell the blind-robust fALL sits
   ...
   fALL column is carried beside every count regardless (D1 convention).

**THE COUNT SURFACE** (correct = certified AND on the true fringe — ORACLE-verified;
mapk banked prior-referenced; full table with every floor's N and error in
`ch_analysis.npz`, figure `ch_surface.png`):

| mix | lit corr/real (wrong) | vlbi corr/real (wrong) | occupancy/925 | fN lit / vlbi (±) |
|---|---|---|---|---|
| m0 (all circular) | **0.70** (0.00) | **0.43** (0.10) | 16 | 4.31 ± 0.37 / 7.59 ± 0.48 |
| m1 e=0.3 | 1.57 (0.03) | 1.13 (0.00) | 23 | 7.39 ± 0.63 / 10.58 ± 0.83 |
| m1 e=0.5 | 3.60 (0.03) | 2.43 (0.03) | 32 | 4.61 ± 0.41 / 9.55 ± 0.76 |
| m1 e=0.7 | **7.83** (0.13) | **6.13** (0.07) | 47 | 8.51 ± 0.72 / 10.64 ± 0.82 |
| m1 eU(0.58) | 4.13 (0.10) | 2.80 (0.10) | 38 | 7.15 ± 0.62 / 10.08 ± 0.80 |
| m2 e=0.3 | 3.27 (0.03) | 1.43 (0.03) | 30 | 9.24 ± 0.76 / 12.13 ± 0.91 |
| m2 e=0.5 | 7.33 (0.23) | 5.30 (0.17) | 48 | 7.83 ± 0.67 / 10.73 ± 0.87 |
| m2 e=0.7 | 8.53 (0.13) | 6.73 (0.07) | 78 | 9.91 ± 0.84 / 11.88 ± 0.99 |
| m2 eU(0.56,0.68) | **8.70** (0.17) | **7.40** (0.00) | 67 | 9.50 ± 0.81 / 11.67 ± 0.93 |
| m3 e=0.3 | 4.57 (0.00) | 1.97 (0.00) | 37 | 9.06 ± 0.76 / 13.52 ± 1.00 |
| m3 e=0.5 | 7.00 (0.03) | 6.40 (0.10) | 64 | 5.71 ± 0.50 / 7.68 ± 0.64 |
| m3 e=0.7 | **5.97** (0.03) | 6.67 (0.03) | **109** | 7.65 ± 0.66 / 7.72 ± 0.65 |
| m3 eU(0.35,0.30,0.68) | 5.23 (0.13) | 5.07 (0.10) | 63 | 5.77 ± 0.51 / 7.58 ± 0.63 |

**Readings.** (1) The all-circular census population sits below onset (0.70/0.43), exactly
the IGNITE-2/SURFACE picture. **Every eccentric mix clears the >1 bar** — a single e = 0.3
member suffices at either tier. (2) At n_ecc = 1 the count is monotone in e (lit: 1.6 → 3.6
→ 7.8), an order of magnitude over the circular baseline at e = 0.7 — *while the mix's own
floor nearly doubles*: the certification gain beats the confusion cost across the whole
measured box. (3) **The trade inverts between n_ecc = 2 and 3 at high e** (`ch_trade.png`):
the surface peaks at m2eU/m2e07 (8.7/8.5 lit) and falls to 6.0 at m3e07 despite three
self-clocking sources — occupancy 109/925 and the ~11× K_counted growth eat the marginal
comb's gain. The capacity crossing is not at the naive occupancy=capacity point but at
~8–12 % occupancy. (4) Wrong-cert rates stay ≤ 0.23/real everywhere (cf. 23/50 realisations
at IGNITE's above-onset cells) — the counts are not purity-bought. fALL-scored counts track
fN counts to ~10 % (surface table, fALL column).
...
**(1) The certified-count-vs-mix surface** — §1 table; every number beside its floor's N
and fit error; fALL column in `ch_analysis.npz`; figure `ch_surface.png`. Headline: the
circular census population is below onset (0.70/0.43); **every eccentric mix clears onset**;
the surface peaks at n_ecc = 2 (8.7 lit / 7.4 vlbi) and inverts by n_ecc = 3 at high e.
...
**(3) The capacity-vs-clock trade curve** — `ch_trade.png`; raw material (occupancy,
capacity, K_sum, active counts) banked per cell. The count rises steeply to ~50–70 active
slots (5–8 % of the 0.4·N·T·ΔF capacity), saturates, and **inverts by 109 slots (12 %)**:
m3e07 posts 6.0/6.7 against m2e07's 8.5/6.7 and m1e07's 7.8/6.1 (lit/vlbi) — the third comb
buys nothing and costs the first two their margin. The binding mechanism at high occupancy
is the K_counted trials term (K_sum grows ~11× from m0 to m3e07) and the finer joint fringe
grid, not the Gumbel floor (which *falls* at m3e07 while the count falls with it).
...
- **vs the ATLAS corner:** ATLAS's single-source requirement for self-clocking was
  ...
  below SURFACE's census-structure onset h\* = −12.50 — **one e = 0.3 loud member already
  turns the certified count on (1.6/1.1 per realisation), and one e = 0.7 member posts
  6–8**. The certification road's requirement relaxes from "louder or longer than anything
  in the modelled box" (IGNITE-2 §4) to **"the population contains one moderately eccentric
  loud member"**. The mixed case does not move ATLAS's absolute-σ_mc corner (that is a
...
- **vs the SCOUT clock:** SCOUT priced certification as a loudness lottery
  ...
  the question is now the fraction of census-class loud sources with e ≳ 0.3–0.5 — an
  astrophysical prior (Taylor/Farr handoff class), not a rarity of loudness draws.
- **vs SURFACE's structure lever:** SURFACE measured 3+13 → 5+11 (loudness structure) as a
  6× count lever at fixed (h, T, tier). CHORUS measures 1-of-16-eccentric (waveform
  structure) as a ~10× lever at the same fixed loudness — **eccentric structure is the
  strongest single lever yet measured in the box** — with the caveat that the two levers
  were measured on different floors (each mix refits its own null, as it must).
...
- **Floors at N = 100** (fresh cells; 150/270 for the reused m0_vlbi bank), fit errors
  0.3–1.0 nat and nullN zero-fractions 0.33–0.81 quoted beside every count; the Gumbel
  point-mass-at-zero caveat (IGNITE-2 §2) applies at the high-zero-fraction lit cells.
